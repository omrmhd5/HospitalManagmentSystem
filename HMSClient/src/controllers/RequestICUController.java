...
public class RequestICUController {
    ...
    private void loadPatients() {
        try {
            ...
            if (patients.isEmpty()) {
                gui.getCmbPatientName().setModel(
                        new DefaultComboBoxModel<>(new String[]{"No patients available"}));
            } else {
                gui.getCmbPatientName().setModel(
                        new DefaultComboBoxModel<>(patients.toArray(new String[0])));
            }

        } catch (RemoteException | NotBoundException ex) {
            Logger.getLogger(RequestICUController.class.getName())
                    .log(Level.SEVERE, null, ex);

            gui.getCmbPatientName().setModel(
                    new DefaultComboBoxModel<>(new String[]{"Error loading patients"}));
        }
    }
    ...
    class SubmitAction implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                ...
                DoctorRequestInterface doctorRequestService =
                        (DoctorRequestInterface) registry.lookup("doctorrequest");

                String doctorEmail = gui.getDoctorEmail();
                String selectedPatient =
                        (String) gui.getCmbPatientName().getSelectedItem();
                String urgency =
                        gui.getUrgencyField().getSelectedItem().toString();
                String diagnosis =
                        gui.getDiagnosisField().getText().trim();
                ...
                String timeHour =
                        (String) gui.getCmbTimeHour().getSelectedItem();
                String timeMinute =
                        (String) gui.getCmbTimeMinute().getSelectedItem();
                String formattedTime = timeHour + ":" + timeMinute;

                String durationHour =
                        (String) gui.getCmbDurationHour().getSelectedItem();
                String durationMinute =
                        (String) gui.getCmbDurationMinute().getSelectedItem();
                String expectedDuration =
                        durationHour + " hours " + durationMinute + " minutes";
                ...
                if (ok) {
                    ...
                    // ✅ NEW: get the real request ID & status from DB
                    int requestID =
                            doctorRequestService.getLastICURequestID();

                    String currentState =
                            icuService.getICURequestStatus(requestID);
    ...
    private boolean validateInput() {
        ...
        if (selectedPatient == null ||
            selectedPatient.equals("No patients available") ||
            selectedPatient.equals("Error loading patients") ||
            selectedPatient.equals("Loading patients...")) {
